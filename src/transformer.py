"""Transform an AST to quad language"""
import builtins
...
class VariablesManager:
    """Manage variables for a run of compiling"""
    ...
    def exists(self, varname):
        """Return true if a variable is a literal or exists"""
        if isinstance(varname, (int, tuple)):
            return True
        try:
...
class AstToQuad:
    """Convert an AST tree of models to quadruple languages

    Works by handling recursively all the models from the root node by all the handle_<model> functions, those functions generally
    return a list of strings (the quadruple code concerning them) and sometimes a variable containing the result of the expression
    """

    def __init__(self, ast: Program):
        """
        Parameters:
            ast: AST generated by the lexer
        """
        self.ast = ast
    ...
    def compute(self):
        """Generate quadruple code for the given ast"""
        self.vm.reset()
        self.reset()
        lines = self.handle_program(self.ast)
        lines.append("HALT")
        return lines, self.failed
    ...
    def handle_input_stmt(self, stmt: InputStmt) -> List[str]:
        ...
        if not self.vm.exists(stmt.id.id):
            raise UnknownVariable(stmt.id.id)
    ...
    def handle_while_stmt(self, stmt: WhileStmt) -> List[str]:
        ...
        # Resolve breaks
        end_of_block = len(slines) + 1 # length of slines + the JUMP line at the end
        for ix, line in enumerate(slines):
            if line == "%BREAK":
                slines[ix] = f"JUMP %+{end_of_block-ix}"
    ...
    def handle_if_stmt(self, stmt: IfStmt) -> List[str]:
        ...
        if not if_lines:
            print("Warning: empty else block")
    ...
    def handle_assignment_stmt(self, stmt: AssignmentStmt) -> List[str]:
        ...
        # Check that we're not assigning a float to an int --> This is forbidden, raise error
        if self.vm.is_float(rvar) and not self.vm.is_float(assign_var):
            raise AssignFloatToInt

        # Check that the variable we're trying to assign exists
        if not self.vm.exists(assign_var):
            raise UnknownVariable(assign_var)
    ...
    def handle_switch_stmt(self, stmt: SwitchStmt) -> List[str]:
        ...
        if not self.vm.exists(switch_var):
            raise UnknownVariable(switch_var)
        ...
        for case in stmt.cases.cases:
            ...
            lines.append(f"INQL {enter_var} {switch_var} {case.num.num}")
            lines.append(f"JMPZ %+{block_offset} {enter_var}")

            lines.extend(block_lines)

        # Add default case
        default_block_lines = self.handle_stmtlist(stmt.default.stmts)
        lines.extend(default_block_lines)

        # Replace breaks
        for ix, line in enumerate(lines):
            if line == "%BREAK":
                lines[ix] = f"JUMP %+{len(lines) - ix}"
